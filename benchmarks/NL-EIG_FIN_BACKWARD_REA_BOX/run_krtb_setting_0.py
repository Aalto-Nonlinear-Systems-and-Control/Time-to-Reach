#!/usr/bin/env python3
"""
KRTB Benchmark Runner for benchmark_NL_EIG_FIN_BACKWARD_REA_BOX
...
r = (1 - x_[0] ** 2 - x_[1] ** 2) ** 2
a = sp.atan(x_[1] / x_[0])
phi0 = x_[0] ** 2 + 2 * x_[1] + x_[1] ** 3
phi1 = x_[0] + sp.sin(x_[1]) + x_[0] ** 3
# phi0 = x_[0] + x_[1] * 1j
# phi1 = x_[0] - x_[1] * 1j
phi = sp.Matrix([phi0, phi1])


def main():
    """Main function for benchmark_NL_EIG_FIN_BACKWARD_REA_BOX KRTB analysis."""
    ...
    config_path = os.path.join(
        ...
        "..",
        "configs",
        "benchmark_NL_EIG_FIN_BACKWARD_REA_BOX.json",
    )
    equilibrium = np.array([0, 0])  # Equilibrium point
    ...
    try:
        ...
        initial_samples = sample_sets(initial_sets, num_samples_initial)
        target_samples = sample_sets(target_sets, num_samples_target)
        ...
        phi_f = sp.lambdify(x_, phi, "numpy")

        ef_initial_final = phi_f(initial_samples[:, 0], initial_samples[:, 1])[
            :, -1, :
        ].T
        ef_target_final = phi_f(target_samples[:, 0], target_samples[:, 1])[:, -1, :].T
        ...
        time_intervals, status = compute_reach_time_bounds(
            ef_target_final, ef_initial_final, -eigenvalues
        )  # for backward reachability, we exchange the initial and target sets, also negative eigenvalues
        ...
        )  # here we did not change the visualization for backward reachability, but the time bounds are correct
