#!/usr/bin/env python3
"""
HJ_Reachability analysis script for benchmark_DUFF_FIN_REA_BALL.
This script runs reachability analysis using HJ_Reachability methods with Ball set support.
"""
...
def main():
    """Main function to run HJ_Reachability analysis for benchmark_NL_EIG_FIN_BACKWARD_UNR_BOX."""
    # Set parameters directly
    config_file = "../../configs/benchmark_NL_EIG_FIN_BACKWARD_UNR_BOX.json"
    ...
    try:
        print("=" * 60)
        print("HJ_Reachability Analysis - benchmark_NL_EIG_FIN_BACKWARD_UNR_BOX")
        print("=" * 60)
        ...
        # Create initial set (now supports Ball sets)
        ...
        if isinstance(initial_set, dict) and initial_set.get("type") == "ball":
            print(
                f"Initial set: Ball with center {initial_set['center']}, radius {initial_set['radius']}"
            )
        else:
            print(f"Initial set bounds: {initial_set}")

        # Create target set (now supports Ball sets)
        ...
        # Display target set information
        if isinstance(target_set, dict) and target_set.get("type") == "ball":
            print(
                f"Target set: Ball with center {target_set['center']}, radius {target_set['radius']}"
            )
        else:
        ...
        # Run analysis with updated function signature
