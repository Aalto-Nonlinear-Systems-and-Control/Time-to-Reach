...
def main():
    """Main function to run HJ_Reachability analysis for benchmark_DUFF_FIN_REA_DISJOINT_BOX."""
    # Set parameters directly
    config_file = "../../configs/benchmark_DUFF_FIN_REA_DISJOINT_BOX.json"
    ...
    try:
        print("=" * 60)
        print("HJ_Reachability Analysis - benchmark_DUFF_FIN_REA_DISJOINT_BOX")
        print("=" * 60)
        ...
        # Create target set (supports multiple disjoint target sets)
        print("Setting up target set...")
        target_sets_config = config["verification"]["target_sets"]
        target_set = create_target_set(target_sets_config)
        if isinstance(target_set, dict) and target_set.get("type") == "union":
            print(f"Multiple target sets detected: {len(target_set['sets'])} sets")
            # Import here to avoid circular imports
            sys.path.insert(0, str(tools_dir))
            from hj.sets import get_set_bounds

            target_bounds = get_set_bounds(target_set)
            print(f"Target set union bounds: {target_bounds}")
        else:
