#!/usr/bin/env python3
"""
KRTB Benchmark Runner for benchmark_NL_SLC_FIN_REA_BOX
...
# for this specific benchmark, we do not need to use path integral to evaluate the eigenfunction
# on specific points, instead, we assume the eigenfunction is given by the user as follows:
x_ = sp.symbols("x:2")
r = (1 - x_[0] ** 2 - x_[1] ** 2) ** 2
a = sp.atan(x_[1] / x_[0])
phi0 = r * sp.exp(a * 1.0j)
phi1 = phi0.conjugate()
# phi0 = x_[0] + x_[1] * 1j
# phi1 = x_[0] - x_[1] * 1j
...
def main():
    """Main function for benchmark_NL_SLC_FIN_REA_BOX KRTB analysis."""
    ...
    config_path = os.path.join(
        ...
        "..",
        "configs",
        "benchmark_NL_SLC_FIN_REA_BOX.json",
    )
    # equilibrium = np.array([0, 0])  # Equilibrium point
    num_samples_initial = 2000  # Reduced for debugging
    num_samples_target = 2000  # Reduced for debugging
    ...
    try:
        ...
        eigenvalues = np.array([lam0, lam0.conjugate()])
